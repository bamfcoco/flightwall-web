...
class CenterByAirport(BaseModel):
  airport: str  # IATA code, e.g. "DTW", "ATL", "LAX"
...
async def api_set_center_airport(payload: CenterByAirport):
  """
  Resolve an airport code (IATA only, from FAA CSV → airports.json)
  into a center lat/lon + display label.

  NOTE: This no longer mutates any global state; the frontend is
